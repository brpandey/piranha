defmodule Piranha.Controller do
  ...
  def make(:booking, timeslot_id, size)
    ...
    Amnesia.transaction do
      ...
      # extract boats from pos 2, 0 based tuple
      :ok = Vessel.update(:boats, elem(result, 2))
      # extract slot from pos 1, 0 based tuple
      :ok = Appointment.update(:slot, elem(result, 1))

      case result do
        {code, _slot, _boats} when code in [:none, :unavailable] ->
          code
