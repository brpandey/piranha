defmodule Piranha.Slot do
  ...
  defstruct id: nil,
            # time interval (which is "detachable", and has same id value as the slot_id)
            interval: nil,
            # availability related
            available: 0,
            customer_count: 0,
            # availability related
            boat_statuses_by_avail: :gb_sets.new(),
            boat_statuses_by_id: %{},
            # boat id retrievals
            boat_ids: []
  ...
  def dump(%Slot{} = s) do
    ...
    # sort boat ids so that they are alphabetical
    ids =
    ...
    %{
      id: s.id,
      start_time: s.interval.unix_start,
  ...
  def register(%Slot{} = slot, %Boat{id: boat_id} = boat) when is_binary(boat_id) do
    case Boat.available?(boat, slot.interval) do
      true ->
        ...
        boat_status = %Status{available: Boat.capacity(boat), customer_count: 0, id: boat_id}
        ...
        slot = %Slot{
          ...
          | available: largest_available,
            # sorted by available spots
            boat_statuses_by_avail: updated_avail,
            # indexed by boat_id to get status
            boat_statuses_by_id: updated_mapping,
            # plain list of boat ids
            boat_ids: updated_ids
        }
  ...
  @spec reserve(t, map, integer) :: {:none, t, map} | {:ok, t, map, list}
  def reserve(slot, fleet, size), do: reserve_while(slot, fleet, size)

  @spec reserve_while(t, map, integer) :: {:none, t, map} | {:ok, t, map, list}
  ...
  @spec reserve_single(t, map, integer) :: :none | {atom, t, map} | {atom, t, map, list}
  ...
  defp availability(%Slot{} = slot, size_request)
    ...
    case :gb_sets.next(iterator) do
      # no boat found that matches size request
      :none ->
        :none

      # found matching boat status
      {%Status{} = value, _iter} ->
        {:ok, value}
    end
  end
