defmodule Piranha.SlotBoat.Test do
  ...
  describe "1 slot 2 boats" do
    ...
    test "requested size in middle of boat availability", %{slot: slot, boats: {b1, b2}} do
      assert 10 = Boat.capacity(b1)
      assert 6 = Boat.capacity(b2)

      # request for size in middle of 6 and 10
      size = 8
    ...
    test "requested size below min of boat availability", %{slot: slot, boats: {b1, b2}} do
      assert 10 = Boat.capacity(b1)
      assert 6 = Boat.capacity(b2)

      # request for size lower than 6 and 10
      size = 4
      fleet = %{b1.id => b1, b2.id => b2}
    ...
    test "no availability, size above max availability", %{slot: slot, boats: {b1, b2}} do
      assert 10 = Boat.capacity(b1)
      assert 6 = Boat.capacity(b2)

      # request for size greater than 6 and 10
      size = 12
      fleet = %{b1.id => b1, b2.id => b2}
  ...
  describe "2 slots 1 boat, overlapping" do
    ...
    test "boat already reserved by first slot", %{slots: {s1, s2}, boat: boat} do
    ...
    test "boat already reserved by second slot", %{slots: {s1, s2}, boat: boat} do
      {:ok, s1, boat} = Slot.register(s1, boat)
      {:ok, s2, boat} = Slot.register(s2, boat)
  ...
  describe "2 slots 2 boats, non-overlapping" do
    ...
    test "medium requested size", %{slots: {s1, s2}, boats: {b1, b2}} do
      assert 10 = Boat.capacity(b1)
      assert 6 = Boat.capacity(b2)

      # request for size below 6 and 10
      size = 8
      fleet = %{b1.id => b1, b2.id => b2}
  ...
  describe "2 slots 2 boats, overlapping" do
    ...
    test "2 slot reserve successes", %{slots: {s1, s2}, boats: {b1, b2}} do
      fleet = %{b1.id => b1, b2.id => b2}
    ...
    test "multiple booking passes over the 2 boats", %{slots: {s1, s2}, boats: {b1, b2}} do
      fleet = %{b1.id => b1, b2.id => b2}
